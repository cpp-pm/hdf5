...
#define H5R_ENCODE_VAR(func, var, size, m) do {             \
    enc_size = buf_size_left;                               \
    if(func(var, size, p, &enc_size) < 0)                   \
        HGOTO_ERROR(H5E_REFERENCE, H5E_CANTENCODE, FAIL, m) \
    if(p && buf_size_left >= enc_size) {                    \
        p += enc_size;                                      \
        buf_size_left -= enc_size;                          \
...
#define H5R_DECODE_VAR(func, var, size, m) do {             \
    dec_size = buf_size_left;                               \
    if(func(p, &dec_size, var, size) < 0)                   \
        HGOTO_ERROR(H5E_REFERENCE, H5E_CANTDECODE, FAIL, m) \
    p += dec_size;                                          \
    buf_size_left -= dec_size;                              \
    buf_size += dec_size;                                   \
...
static herr_t H5R__encode_obj_token(const void *obj_token, size_t token_size, unsigned char *buf, size_t *nalloc);
static herr_t H5R__decode_obj_token(const unsigned char *buf, size_t *nbytes, void *obj_token, uint8_t *token_size);
 ...
 */
herr_t
H5R__create_object(const void *obj_token, size_t token_size, struct href *ref)
{
    size_t encode_size;
    ...
    HDmemcpy(ref->ref.obj.token, obj_token, token_size);
    ...
    H5R_LOG_DEBUG("Created object reference, %d, filename=%s, obj_addr=%s, encode size=%u",
        (int)sizeof(struct href), ref->ref.obj.filename, H5R__print_token(ref->ref.obj.token),
        ref->encode_size);
 ...
 */
herr_t
H5R__create_region(const void *obj_token, size_t token_size, H5S_t *space,
    struct href *ref)
{
    size_t encode_size;
    herr_t ret_value = SUCCEED; /* Return value */
 ...
 */
herr_t
H5R__create_attr(const void *obj_token, size_t token_size,
    const char *attr_name, struct href *ref)
{
    size_t encode_size;
    herr_t ret_value = SUCCEED; /* Return value */
...
H5R__copy(const struct href *src_ref, struct href *dest_ref)
{
    ...
    HDmemcpy(dest_ref->ref.obj.token, src_ref->ref.obj.token, src_ref->token_size);
    dest_ref->encode_size = src_ref->encode_size;
    dest_ref->type = src_ref->type;
    dest_ref->token_size = src_ref->token_size;
    ...
    if(src_ref->loc_id == H5I_INVALID_HID) {
        ...
        dest_ref->ref.obj.filename = NULL;
 ...
 */
herr_t
H5R__get_obj_token(const struct href *ref, void *obj_token, size_t *token_size)
{
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    if(obj_token) {
        ...
        HDmemcpy(obj_token, ref->ref.obj.token, ref->token_size);
    }
    if(token_size)
 ...
 */
herr_t
H5R__encode(const char *filename, const struct href *ref, unsigned char *buf,
    size_t *nalloc, unsigned flags)
{
    ...
    if(flags & H5R_IS_EXTERNAL) {
        /* Encode file name */
        H5R_ENCODE(H5R__encode_string, filename, "Cannot encode filename");
...
H5R__decode(const unsigned char *buf, size_t *nbytes, struct href *ref)
{
    ...
    H5R_LOG_DEBUG("Decoded reference, %d, filename=%s, obj_addr=%s, encode size=%u",
        (int)sizeof(struct href), ref->ref.obj.filename, H5R__print_token(ref->ref.obj.token),
        ref->encode_size);
 ...
 */
static herr_t
H5R__encode_obj_token(const void *obj_token, size_t token_size,
    unsigned char *buf, size_t *nalloc)
{
    ...
    if(buf && *nalloc >= token_size) {
        uint8_t *p = (uint8_t *)buf;
        const uint8_t *t = (const uint8_t *)obj_token;
        unsigned int q = (unsigned int) (token_size / H5_SIZEOF_UINT64_T);
        unsigned int r = token_size % H5_SIZEOF_UINT64_T;
        ...
        *p++ = (uint8_t)(token_size & 0xff);
        ...
        /* Encode token */
        while(q--) {
            UINT64ENCODE(p, *(uint64_t *)t);
            t += H5_SIZEOF_UINT64_T;
        }
        while(r--)
            *p++ = *t++;
    }
...
H5R__decode_obj_token(const unsigned char *buf, size_t *nbytes,
    void *obj_token, uint8_t *token_size)
{
    const uint8_t *p = (const uint8_t *)buf;
    uint8_t *t = (uint8_t *)obj_token;
    unsigned int q, r;
    herr_t ret_value = SUCCEED;
    ...
    *token_size = *p++;
    ...
    q = (unsigned int) (*token_size / H5_SIZEOF_UINT64_T);
    r = *token_size % H5_SIZEOF_UINT64_T;
    while(q--) {
        UINT64DECODE(p, *(uint64_t *)t);
        t += H5_SIZEOF_UINT64_T;
    }
    while(r--)
        *t++ = *p++;
...
H5R__encode_obj_addr_compat(haddr_t addr, unsigned char *buf, size_t *nalloc)
{
    ...
    *nalloc = sizeof(addr);
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5R__encode_obj_addr_compat() */
