    ...
    href_t *ref_ptr)
{
    ...
    /* Create the reference (do not pass filename, since file_id is attached) */
    HDmemset(ref_ptr, 0, H5R_REF_BUF_SIZE);
    if(H5R__create_region(obj_token, cont_info.token_size, space, ref) < 0)
    ...
    href_t *ref_ptr)
{
    ...
    /* Create the reference (do not pass filename, since file_id is attached) */
    HDmemset(ref_ptr, 0, H5R_REF_BUF_SIZE);
    if(H5R__create_attr(obj_token, cont_info.token_size, attr_name, ref) < 0)
...
H5Ropen_object(const href_t *ref_ptr, hid_t oapl_id)
{
    ...
    /* Get object token */
    if(H5R__get_obj_token(ref, obj_token, NULL) < 0)
...
H5Rget_file_name(const href_t *ref_ptr, char *name, size_t size)
{
    ...
    if(H5I_INVALID_HID == (loc_id = H5R__get_loc_id(ref))) {
        ...
        if(H5VL_file_get(vol_obj, H5VL_FILE_GET_NAME, H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL, H5I_FILE, size, name, &ret_value) < 0)
...
H5Rget_obj_name(const href_t *ref_ptr, hid_t oapl_id, char *name, size_t size)
{
    ...
    /* Get object token */
    if((ret_value = H5R__get_obj_token(ref, obj_token, NULL)) < 0)
