...
main(void) {
    ...
    /* Create reference to dataset1 */
    H5Rcreate(&ref_buf[0], file1, "dataset1", H5R_OBJECT, H5I_INVALID_HID);

    /* Store reference in separate dataset */
    ...
    /* Access reference and read dataset data */
    assert(H5Rget_type(&new_ref_buf[0]) == H5R_OBJECT2);
    H5Rget_obj_type3(&new_ref_buf[0], H5P_DEFAULT, &obj_type);
    assert(obj_type == H5O_TYPE_DATASET);
    dset1 = H5Ropen_object(&new_ref_buf[0], H5P_DEFAULT);
